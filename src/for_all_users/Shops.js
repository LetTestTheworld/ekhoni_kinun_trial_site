...
const Shops = () => {
    ...
    const [noShopFoundMsg, setNoShopFoundMsg] = useState('');
    ...
    function searchShops(param) {
        if (param.length > 0) {
            ...
            setNoShopFoundMsg(filteredVendors.length === 0 ? 'No Shop Found' : '');
        } else {
            setSearchedVendors("")
            setNoShopFoundMsg('')
            handleSearchShops()
    ...
    function handleFilter() {
        ...
        if (selectCategory !== 'all-shops') {
            filteredVendors = vendors.filter((vendor) =>
                Number(vendor.Shop_type) === Number(selectCategory)
            );
        } else {
            filteredVendors = [...vendors];
        }

        // If there's a search term, apply it to the filtered results
        ...
        setSearchResult([]); // Clear search results when filtering
        setNoShopFoundMsg(filteredVendors.length === 0 ? 'No Shop Found' : '');
    }

    function handleSearchShops() {
        let filteredVendors = filterResult.length > 0 ? filterResult : vendors;

        if (filterResult.length > 0 && searchTerm.length > 0) {
            filteredVendors = filteredVendors.filter((vendor) => vendor.shop_name.toLowerCase().includes(searchTerm.toLowerCase()));
