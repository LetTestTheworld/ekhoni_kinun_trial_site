...
import usePlaceOrder from "./operations/usePlaceOrder";
...
function ProductDetailsPageTwo() {
    const [quantity, setQuantity] = useState(1);
    const [showCheckout, setShowCheckout] = useState(false);
    const [respondedMessage, setRespondedMessage] = useState('');
    const param = useParams();
    const user = UseUser();
    const productAndCategory = useProductAndCategory();
    const { addToCart } = CartOperations(); // ✅ Correct: Top-level hook call

    const product_details = productAndCategory.allProducts.find(
        product => product.product_slug === param.product_slug
    );

    const categoryWiseProducts = productAndCategory.allProducts.filter(
        product => product.product_category === product_details?.product_category
    );

    // Handle order placementconst [showModal, setShowModal] = useState(false);
                                        ...
                                        <div class="product-meta mb-4">
                                            ...
                                            <div class="product-category">
                                                <small class="text-muted">Category:
                                                    ...
                                                    <p href="#" class="text-primary d-inline ms-2">{productAndCategory?.allCategories?.find(category => category.category_id === product_details?.category_id
                                                    )?.category_name
